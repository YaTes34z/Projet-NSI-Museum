...
personnage = pygame.image.load('images/homme_droit_1.png')  # Remplace par le chemin de ton image
...
class Ennemi:
    ...
    def ennemiIA(self, light_x, light_y, light_radius, dt):
        """Gère le mouvement des ennemis en fonction de la lumière."""
        distance_to_light = ((self.x - light_x) ** 2 + (self.y - light_y) ** 2) ** 0.5
        if distance_to_light < light_radius:
            self.time_in_light += dt
            dx = self.x - light_x
            dy = self.y - light_y
            angle = math.atan2(dy, dx)
        ...
        if is_point_in_cone(moisissure_screen_x, moisissure_screen_y, cone_points):
            ...
            if distance < 75:  # Si la souris est suffisamment proche de la moisissure
                if pygame.mouse.get_pressed()[0]:  # Si le bouton gauche de la souris est enfoncé
                    ...
                    elif temps_actuel - nettoyage_temps_debut[moisissure] >= 2000:  # 3000 ms = 3 secondes
...
while running:
    ...
    for ennemi in ennemis[:]:
        ennemi.ennemiIA(x, y, cone_length, dt)
